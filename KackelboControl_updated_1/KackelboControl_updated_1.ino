...
bool failedSyncingTime;
int failedTimeSyncAttempts = 0;
bool failedPostingTemp;
int failedTempPostAttempts = 0;
bool failedSyncingTriggers;
int failedTriggerSyncAttempts = 0;
...
void loop() {
  ...
  if (failedTimeSyncAttempts > 3 || failedTempPostAttempts > 3 || failedTriggerSyncAttempts > 3) {
    WiFi.disconnect();
    SetUpWifi();
  }

  currentMillis = millis();
  if ((currentMillis - previousTriggerMillis >= syncTriggersInterval) || failedSyncingTriggers) {
    GetTriggerInformation();
  }

  currentMillis = millis();
  if ((currentMillis - previousTimeSyncMillis >= timeSyncInterval) || failedSyncingTime) {
    TimeSync();
  }
...
void TimeSync() {
  ...
  if (statusCode == 200) {
    ...
    failedSyncingTime = false;
    failedTimeSyncAttempts = 0;

  } else {
    Serial.println("Error, Statuscode:");
    Serial.println(statusCode);
    failedSyncingTime = true;
    failedTimeSyncAttempts++;
    String response = httpClient.responseBody();
  }
...
void GetTriggerInformation() {
  ...
  if (statusCode == 200) {
    ...
    failedSyncingTriggers = false;
    failedTriggerSyncAttempts = 0;
  } else {
    Serial.println("Error, Statuscode:");
    Serial.println(statusCode);
    String response = httpClient.responseBody();
    failedSyncingTriggers = true;
    failedTriggerSyncAttempts++;
  }
...
void PostSensorValues() {
  ...
  if (((currentMinute == 0 || currentMinute == 30) && intCurrentTime != lastSentSensorValues) || failedPostingTemp) {
    ...
    if (tempIndoorCelsius <= -10 || tempIndoorCelsius >= 100) {
      inWidth = 6;
    } else if (tempIndoorCelsius >= 0 && tempIndoorCelsius < 10) {
      inWidth = 4;
    }

    if (tempOutdoorCelsius <= -10 || tempOutdoorCelsius >= 100) {
      outWidth = 6;
    } else if (tempOutdoorCelsius >= 0 && tempOutdoorCelsius < 10) {
      outWidth = 4;
    }
    ...
    if (statusCode == 200) {
      ...
      failedPostingTemp = false;
      failedTempPostAttempts = 0;
    } else {
      Serial.println("Error, Statuscore:");
      Serial.println(statusCode);
      failedPostingTemp = true;
      failedTempPostAttempts++;
    }
